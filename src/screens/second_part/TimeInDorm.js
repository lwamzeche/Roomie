...
const TinDorm = ({navigation}) => {
  const [selectedGamingHabit, setSelectedTimingHabit] = useState(null);
  const {width} = useWindowDimensions();
  const TimingHabits = ['All the time', 'Rare', 'Sometimes', 'Does not matter'];

  const selectTimingHabit = habit => {
    setSelectedTimingHabit(habit);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subTitle}>Amount of Time Spent in Dorm</Text>

      <View style={styles.choicesContainer}>
        {TimingHabits.map(habit => (
          <ChoiceButton
            key={habit}
            label={habit}
            isSelected={selectedGamingHabit === habit}
            onSelect={() => selectTimingHabit(habit)}
