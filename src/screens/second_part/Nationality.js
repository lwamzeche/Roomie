...
const Nationality = ({navigation}) => {
  const [selectedGamingHabit, setSelectedNationality] = useState(null);
  const {width} = useWindowDimensions();
  const Nationality = ['Domestic', 'International', 'Does not matter'];

  const selectNationality = habit => {
    setSelectedNationality(habit);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subTitle}>Their Nationality</Text>

      <View style={styles.choicesContainer}>
        {Nationality.map(habit => (
          <ChoiceButton
            key={habit}
            label={habit}
            isSelected={selectedGamingHabit === habit}
            onSelect={() => selectNationality(habit)}
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('SurveySuccess')}>
